import torch
import torch.nn as nn
import torch.nn.functional as F


class GraphCapsuleLayer(nn.Module):
    """图胶囊层"""

    def __init__(self, in_dim: int, out_dim: int, num_iterations: int = 3):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.num_iterations = num_iterations
        self.eps = 1e-8

        # 投影矩阵
        self.W = nn.Parameter(torch.Tensor(in_dim, out_dim))
        nn.init.xavier_uniform_(self.W)

        # 注意力参数
        self.attn_a = nn.Parameter(torch.Tensor(2 * out_dim, 1))
        nn.init.xavier_uniform_(self.attn_a)

    def forward(self, h: torch.Tensor, adj: torch.Tensor, edge_type_mask=None) -> torch.Tensor:
        """
        前向传播
        Args:
            h: 节点特征 [num_nodes, in_dim]
            adj: 归一化邻接矩阵 [num_nodes, num_nodes]
            edge_type_mask: 边类型掩码（可选）
        Returns:
            v: 输出胶囊 [num_nodes, out_dim]
        """
        num_nodes = h.shape[0]
        device = h.device

        # Step 1: 计算所有节点的投影投票
        u_hat = torch.matmul(h, self.W)  # [num_nodes, out_dim]

        # Step 2: 初始化log priors
        b = torch.zeros(num_nodes, num_nodes).to(device)

        # Step 3: 动态路由迭代
        for t in range(self.num_iterations):
            # 计算路由系数
            c = F.softmax(b, dim=1)  # [num_nodes, num_nodes]

            # 计算加权输入胶囊
            s = torch.matmul(c.T, u_hat)  # [num_nodes, out_dim]

            # Squash激活
            v = self._squash(s)

            # 更新log priors（使用余弦相似度）
            if t < self.num_iterations - 1:
                v_norm = F.normalize(v, dim=1)
                u_hat_norm = F.normalize(u_hat, dim=1)
                agreement = torch.matmul(v_norm, u_hat_norm.T)  # [num_nodes, num_nodes]
                b = b + agreement

        # Step 4: 注意力机制
        h_i = h.unsqueeze(1).expand(-1, num_nodes, -1)
        h_j = h.unsqueeze(0).expand(num_nodes, -1, -1)
        h_concat = torch.cat([h_i, h_j], dim=2)

        attn_scores = torch.matmul(h_concat, self.attn_a).squeeze(2)
        attn_scores = F.leaky_relu(attn_scores, negative_slope=0.2)

        # 应用邻接矩阵掩码
        attn_weights = F.softmax(attn_scores * adj, dim=1)

        # Step 5: 融合路由和注意力
        lambda_weight = 0.6
        combined_weights = lambda_weight * c + (1 - lambda_weight) * attn_weights
        s_final = torch.matmul(combined_weights.T, u_hat)
        v_final = self._squash(s_final)

        return v_final

    def _squash(self, s: torch.Tensor) -> torch.Tensor:
        """Squash激活函数"""
        s_norm = torch.norm(s, dim=1, keepdim=True)
        scale = s_norm / (1 + s_norm + self.eps)
        return scale * s / (s_norm + self.eps)


class GraphCapsuleNetwork(nn.Module):
    """图胶囊网络"""

    def __init__(self,
                 in_dim: int,
                 hidden_dims: List[int],
                 num_iterations: int = 3):
        super().__init__()

        self.layers = nn.ModuleList()
        prev_dim = in_dim

        for hidden_dim in hidden_dims:
            self.layers.append(
                GraphCapsuleLayer(prev_dim, hidden_dim, num_iterations)
            )
            prev_dim = hidden_dim

    def forward(self, x: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        """前向传播"""
        for layer in self.layers:
            x = layer(x, adj)
        return x